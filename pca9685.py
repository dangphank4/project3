...
GPIOCHIP_PCA9685_OE = "/dev/gpiochip1"
GPIOLINE_PCA9685_OE = 8

chip = gpiod.Chip(GPIOCHIP_PCA9685_OE)
cfg  = gpiod.LineSettings(direction=Direction.OUTPUT, output_value=Value.ACTIVE)
req  = chip.request_lines(consumer="pca9685", config={GPIOLINE_PCA9685_OE: cfg})
...
# todo reset
class PCA9685:
    def __init__(self, bus=1, use_extclk=False):
        self.use_extclk = use_extclk
        if use_extclk:
            self.extclk = 24.567e6
        else:
            self.extclk = 25.000e6
    ...
    def output_enable(self):
        req.set_value(GPIOLINE_PCA9685_OE, Value.INACTIVE)
        return

    # set OE pin HIGH
    def output_disable(self):
        req.set_value(GPIOLINE_PCA9685_OE, Value.ACTIVE)
        return
